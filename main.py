...
async def upload(bot: Client, m: Message):
    ...
    platform = input4.text.strip()
    await input4.delete(True)
    MR = ""

    if platform == "1":
        await editable.edit("Send PW Token Only")

        token_msg: Message = await bot.listen(editable.chat.id)
        MR = token_msg.text.strip()
    ...
    # Auto detect Classplus links
    if "classplus" in text.lower():
        MR = "CLASSPLUS"
""
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            if "visionias" in url:
             ...
             id =  url.split("/")[-2]
             url = f"https://anonymouspwplayer-554b25895c1a.herokuapp.com/pw?url={url}&token={MR}"
